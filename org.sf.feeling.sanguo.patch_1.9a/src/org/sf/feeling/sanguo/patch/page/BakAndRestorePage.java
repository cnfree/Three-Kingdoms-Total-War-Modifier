...
public class BakAndRestorePage extends SimpleTabPage implements Listener {
	...
	private void createPatchArea() {
		TableWrapData td;
		Section bakSection = WidgetUtil.getToolkit().createSection(
		...
		Composite bakComposite = WidgetUtil.getToolkit().createComposite(
				bakSection, SWT.NONE);
		...
		initBakButton = WidgetUtil.getToolkit().createButton(bakComposite,
				"设置当前游戏数据作为原始备份档（请务必保证当前游戏数据无异常状态）", SWT.CHECK);
		GridData gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.horizontalSpan = 2;
		initBakButton.setLayoutData(gd);

		initApplyBakButton = WidgetUtil.getToolkit().createButton(bakComposite,
				SWT.PUSH, true);
		...
		final Button bakButton = WidgetUtil.getToolkit().createButton(
				bakComposite, "备份当前游戏数据", SWT.CHECK);

		final Text description = WidgetUtil.getToolkit().createText(
				bakComposite, "请输入备份描述", SWT.NONE);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.widthHint = 200;
		description.setLayoutData(gd);
		description.setEnabled(false);

		final Button applyBakButton = WidgetUtil.getToolkit().createButton(
				bakComposite, SWT.PUSH, true);
		...
		final Button clearBakButton = WidgetUtil.getToolkit().createButton(
				bakComposite, "删除全部历史备份记录（不包含原始档）", SWT.CHECK);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.widthHint = 200;
		gd.horizontalSpan = 2;
				...
				if (folder.exists() && folder.isDirectory()) {
					...
					if (children != null) {
						for (int i = 0; i < children.length; i++) {
							...
							if (file.isFile()
									&& !file.getAbsolutePath().equals(
											BakUtil.defalutResourceBakFilePath)) {
								file.delete();
							}
						}
					}
		...
		restoreButton = WidgetUtil.getToolkit().createButton(bakComposite,
				"还原游戏数据至原始档状态（视机器性能不同，可能需要等待几分钟）", SWT.CHECK);
		gd = new GridData(GridData.FILL_HORIZONTAL);
		gd.widthHint = 200;
		gd.horizontalSpan = 2;
		restoreButton.setLayoutData(gd);

		applyRestoreButton = WidgetUtil.getToolkit().createButton(bakComposite,
				SWT.PUSH, true);
		...
		Section restoreSection = WidgetUtil.getToolkit().createSection(
				container.getBody(), Section.EXPANDED);
		td = new TableWrapData(TableWrapData.FILL);
		...
		Composite restoreComposite = WidgetUtil.getToolkit().createComposite(
				restoreSection, SWT.NONE);
		...
		final Table table = WidgetUtil.getToolkit().createTable(
				restoreComposite,
		...
		String[] columns = new String[] { "备份时间", "描述" };
		...
		restoreBakButton = WidgetUtil.getToolkit().createButton(buttonGroup,
				"使用选中备份文件还原", SWT.CHECK);
		gd = new GridData();
		gd.widthHint = 300;
		restoreBakButton.setLayoutData(gd);

		applyRestoreBakButton = WidgetUtil.getToolkit().createButton(
				buttonGroup, SWT.PUSH, true);
		...
		applyRestoreBakButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent e) {
				if (bakFileTable.getSelection() instanceof StructuredSelection) {
					StructuredSelection selection = ((StructuredSelection) bakFileTable
							.getSelection());
		...
		restoreBakButton.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				applyRestoreBakButton.setEnabled(restoreBakButton
						.getSelection());
			}
		});

		final Button deleteBakButton = WidgetUtil.getToolkit().createButton(
				buttonGroup, "删除选中备份文件", SWT.CHECK);
		gd = new GridData();
		gd.widthHint = 300;
	...
	private void refreshTable() {
		if (bakFileTable != null && bakFileTable.getTable() != null
