...
public class SWTGraphics
{
	...
	static class RectangleClipping implements Clipping
	{
		...
		public void setOn( GC gc, int translateX, int translateY )
		{
			...
			gc.setClipping( xInt + translateX,
					yInt + translateY,
	...
	public void clipRect( Rectangle rect )
	{
		...
		checkSharedClipping( );
		currentState.relativeClip.intersect( rect.x, rect.y, rect.x
	...
	{
		checkGC( );
		gc.drawImage( srcImage, x1, y1, w1, h1, x2 + translateX, y2
				+ translateY, w2, h2 );
	}
	...
	{
		checkPaint( );
		gc.drawRoundRectangle( r.x + translateX,
				r.y + translateY,
				r.width,
				r.height,
				arcWidth,
	...
	{
		checkPaint( );
		gc.fillGradientRectangle( x + translateX,
				y + translateY,
				w,
				h,
				vertical );
	...
	{
		checkFill( );
		gc.fillRoundRectangle( r.x + translateX,
				r.y + translateY,
				r.width,
				r.height,
				arcWidth,
	...
	private static void reconcileHints( GC gc, int applied, int hints )
	{
		...
		if ( changes != 0 )
		{
			...
			{
				if ( ( hints & ADVANCED_GRAPHICS_MASK ) != 0
						&& !gc.getAdvanced( ) )
				{
					gc.setAdvanced( true );
				}
	...
	protected void restoreState( State s )
	{
		...
		if ( gc.getAdvanced( )
				&& ( s.graphicHints & ADVANCED_GRAPHICS_MASK ) == 0 )
		{
	...
	public void shear( float horz, float vert )
	{
		...
		transform.setElements( matrix[0] + matrix[2] * vert, matrix[1]
				+ matrix[3]
				* vert, matrix[0] * horz + matrix[2], matrix[1]
				* horz
	...
	static LineAttributes clone( LineAttributes src )
	{
		...
		return new LineAttributes( src.width,
				src.cap,
				src.join,
				src.style,
				dashClone,
