...
public class ColorSelector
{
	...
	public void open( )
	{
		...
		colorDialog.setText( "选择颜色" ); //$NON-NLS-1$
	...
	void fireModifyEvent( final Event event )
	{
		if ( fButton == null || fButton.isDisposed( ) || !fButton.isEnabled( ) )
			return;
		if ( listeners != null )
		{
			for ( int i = 0; i < listeners.size( ); i++ )
			{
				final ModifyEvent selectEvent = new ModifyEvent( event );
				final ModifyListener listener = (ModifyListener) listeners.get( i );
				Display.getDefault( ).asyncExec( new Runnable( ) {

					public void run( )
					{
						listener.modifyText( selectEvent );
					}
				} );
			}
		}
