...
public class GeneralAgeUtil {
	...
	public static int[] computeGeneralAge(String generalCode) {
		...
		if (generalRelative != null && generalRelative.length == 3) {
			...
			int currentAge = Integer.parseInt((String) generalAgeMap
					.get(generalCode));
			...
			if (children != null && !children.isEmpty()) {
				for (int i = 0; i < children.size(); i++) {
					int age = Integer.parseInt((String) generalAgeMap
							.get((String) children.get(i))) + 12;
					if (age > minAge)
						minAge = age;
				}
			}
			...
			return new int[] { currentAge, minAge, maxAge };
		}
		return null;
	}
		...
		if (FileConstants.stratFile.exists()) {
			try {
				...
				BufferedReader in = new BufferedReader(new InputStreamReader(
						new FileInputStream(FileConstants.stratFile), "GBK"));
				String parent = null;
				...
				in.close();
				return new Object[] { parent, children, brothers };
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
				...
				while ((line = in.readLine()) != null) {
					if (!startFaction) {
						Pattern pattern = Pattern
								.compile("^\\s*(faction)(\\s+)(\\S+)(\\s*)(,)");
						Matcher matcher = pattern.matcher(line);
						if (matcher.find()) {
							if (faction.equalsIgnoreCase( matcher.group().replaceAll(
									"faction", "").replaceAll(",", "").trim())) {
								startFaction = true;
								continue;
							}
						}
						...
						if (matcher.find()) {
							Pattern pattern1 = Pattern
									.compile("(named)(\\s+)(character)(\\s*)(,)");
							Matcher matcher1 = pattern1.matcher(line);
							if (matcher1.find()) {
								String[] value = matcher.group().split(",");
								String general = value[value.length - 2]
										.replaceAll("character", "").trim();

								String[] splits = line.trim().split(",");
								for (int i = 0; i < splits.length; i++) {
									String split = splits[i];
									if (split.toLowerCase().indexOf("age") > -1) {
										factionGeneralAgeMap.put(general, split
						...
						if (matcher.find()) {
							...
							String general = splits[0].trim().split("\\s+")[1]
									.trim();
							for (int i = 0; i < splits.length; i++) {
								String split = splits[i];
								if (split.toLowerCase().indexOf("age") > -1) {
	...
	public static void convertFactionAges(String[] factions) {
		...
		for (int j = 0; j < factions.length; j++) {
			...
			List factionGenerals = (List) MapUtil.factionGeneralMap
					.get(faction);
			for (int i = 0; i < factionGenerals.size(); i++) {
				String general = (String) factionGenerals.get(i);
				convert16YearGeneralAge(factionAgeMap, general,
						generalRelativeMap, generalAgeMap);
			}
			...
			for (int i = 0; i < factionGenerals.size(); i++) {
				...
				if (!factionAgeMap.containsKey(general)) {
					relativeGeneralAgeMap.put(general, generalAgeMap
							.get(general));
				}
			}
			...
			Collections.sort(relativeGeneralAgeMap.getKeyList(),
					new Comparator() {
						...
						public int compare(Object o1, Object o2) {
							return ((String) generalAgeMap.get(o1))
									.compareTo(((String) generalAgeMap.get(o2)));
						}
					});
				...
				if (generalRelative != null && generalRelative.length == 2) {
					...
					if (youngBrother != null) {
						if (factionAgeMap.containsKey(youngBrother)) {
							int brotherAge = Integer
									.parseInt((String) factionAgeMap
											.get(youngBrother));
							if (brotherAge + 1 > minAge)
								minAge = brotherAge + 1;
						} else {
							int brotherAge = Integer
									.parseInt((String) generalAgeMap
											.get(youngBrother));
							if (brotherAge + 1 > minAge)
								minAge = brotherAge + 1;
						}
			...
			try {
				...
				PrintWriter out = new PrintWriter(new BufferedWriter(
						new OutputStreamWriter(new FileOutputStream(
								FileConstants.stratFile), "GBK")), false);
	...
	private static Object[] computeGeneralRelative(String general,
		...
		return new Object[] { children, youngBrother };
	}
