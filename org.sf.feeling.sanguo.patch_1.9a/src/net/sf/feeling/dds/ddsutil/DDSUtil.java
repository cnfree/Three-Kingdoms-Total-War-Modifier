...
import net.sf.feeling.dds.model.SingleTextureMap;
import net.sf.feeling.dds.model.TextureMap;
...
public class DDSUtil
{
	...
	/**
	 * Create a {@link ImageData} from a DXT-compressed dds-texture
	 ...
	 * @throws IOException
	 */
	public static ImageData decompressTexture( final File file )
			throws IOException
	{
	...
	/**
	 * Create a {@link ImageData} from a DXT-compressed {@link DDSImage}
	 * 
	 * @param ddsimage
	 * @return
	 */
	public static ImageData decompressTexture( final DDSImage ddsimage )
	{

		return decompressTexture( ddsimage.getMipMap( 0 ).getData( ),
				ddsimage.getWidth( ),
				ddsimage.getHeight( ),
	...
	/**
	 * Create a {@link ImageData} from a DXT-compressed Byte-array.
	 * 
	 ...
	 * @return
	 */
	public static ImageData decompressTexture( final byte[] compressedData,
			final int width, final int height,
			final Squish.CompressionType compressionType )
	{

		return new DXTBufferDecompressor( compressedData,
				width,
				height,
	...
	/**
	 * Create a {@link ImageData} from a DXT-compressed ByteBuffer.
	 * 
	 * @param textureBuffer
	 * @param width
	 * @param height
	 ...
	 * @return
	 */
	public static ImageData decompressTexture( final ByteBuffer textureBuffer,
			final int width, final int height,
			final Squish.CompressionType compressionType )
	{

		return new DXTBufferDecompressor( textureBuffer,
				width,
				height,
				compressionType ).getImage( );
	}
	...
	/**
	 * Compresses a {@link ImageData} into a {@link ByteBuffer}
	 * 
	 * @param image
	 * @param compressionType
	 * @return
	 */
	public static ByteBuffer compressTexture( final ImageData image,
			final Squish.CompressionType compressionType )
	{
	 ...
	 * @return
	 */
	public static byte[] compressTextureToArray( final ImageData image,
			final Squish.CompressionType compressionType )
	{
	...
	public static void write( final File destinationfile,
			ImageData sourceImage, final int pixelformat,
			boolean generateMipMaps ) throws IOException
	{
		...
		{
			sourceImage = ImageUtils.convertToRGBA( sourceImage );
		}
	 ...
	 * 
	 */
	public void write( final File file, TextureMap map, final int pixelformat )
			throws IOException
	{

		writeDDSImage( file,
				map.getDXTCompressedBuffer( pixelformat ),
				map.getWidth( ),
				map.getHeight( ),
				pixelformat );
	}
	...
	private static DDSImage writeDDSImage( final File file,
			ByteBuffer[] mipmapBuffer, final int width, final int height,
			final int pixelformat ) throws IllegalArgumentException,
			IOException
	{

		DDSImage writedds = DDSImage.createFromData( pixelformat,
				width,
				height,
				mipmapBuffer );
		// writedds.debugPrint();
	 ...
	 * TODO The {@link DDSImage} specifies isCompressed even on D3DFMT_A8R8G8B8,
	 * D3DFMT_R8G8B8 and D3DFMT_X8R8G8B8 this doesn't
	 * 
	 * @param pixelformat
	 *            DDSImage pixelformat
	...
	private static Squish.CompressionType findCompressionFormat(
			DDSImage ddsimage )
	{
	 ...
	 * @return
	 */
	public static Squish.CompressionType getSquishCompressionFormat(
	...
	/**
	 * Convert Integer-CompressionType of {@link DDSImage} to {@link Squish}
	 * -Enum
	 * 
	 * @param compressionType
	 *            int
	 ...
	 */
	public static Squish.CompressionType selectedCompression(
