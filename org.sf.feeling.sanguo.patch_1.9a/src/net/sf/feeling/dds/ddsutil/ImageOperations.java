...
//import java.awt.image.ImageData;
...
/**
 * General image functions for handling Images, ImageDatas and Arrays
 * @author danielsenff
 *
 */
...
//		System.out.println((c & 0xff000000) >> 24);
	 ...
	 * This is for {@link ImageData}
	 * @param bi
	 * @param channel
	 * @param color
	 * @return
	 */
	public static byte[] paintValueInChannel(final ImageData bi, final int channel, final float color) {
		return paintChannelInValue(ImageUtils.convertBIintoARGBArray(bi), 
				bi.width, 
				bi.height, 
				channel, color);
	}
	 ...
	 * @return
	 */
	public static ImageData writeBufferToRaster(final Buffer buffer, final ImageData raster) {
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				...
				RGB rgb1= new RGB(rgb[0], rgb[1],rgb[2]);
				raster.setPixel(x, y, raster.palette.getPixel( rgb1 ));
				raster.setAlpha( x, y, rgb[3] );
			}
		}
		return raster;
	}
